...
- 确认 SSH 端口未被屏蔽
- 再次检查用户名和密码 / SSH 密钥
- 尝试手动连接：`ssh root@your-vps-ip`

### 部署失败

- 查看应用中的日志获取具体错误信息
- 确认 VPS 运行 Ubuntu 22.04+ 或 Debian 11+
- 确认有 root 或 sudo 权限
- 在 VPS 上手动运行 `apt update` 检查
...
- 确认 IPv4 转发已启用：`sysctl net.ipv4.ip_forward` 应为 `1`
- 确认 iptables 规则已生效
- **最重要的一点：检查 VPS 服务商的防火墙/安全组。** 很多服务商（AWS、DigitalOcean、Vultr、阿里云、腾讯云等）有外部防火墙默认屏蔽 UDP 端口，需手动开放 UDP 51820（或自定义端口）。
...
- Verify that SSH port is not blocked
...
- **Most importantly: Check your VPS provider's firewall / security group.** Many providers (AWS, DigitalOcean, Vultr, Alibaba Cloud, Tencent Cloud) have an external firewall that blocks UDP ports by default. You must add a rule to allow UDP port 51820 (or your custom WireGuard port).
...
每个人都该有选择的权利 — MenZenithRBowstringS

Everyone deserves the right to choose — MenZenithRBowstringS
